...
class DataBaseHelper( context: Context) : SQLiteOpenHelper(context,"expensetraker.db",null,1){
    override fun onCreate(db: SQLiteDatabase?) {
        db?.execSQL("CREATE TABLE monthlyExpenses(_id integer primary key autoincrement,item_name TEXT,amount number,description text,date_added text )")
       db?.execSQL("CREATE TABLE monthlyIncome(_idIncome integer primary key autoincrement,income_source TEXT,income_amount number,date_added text )")
